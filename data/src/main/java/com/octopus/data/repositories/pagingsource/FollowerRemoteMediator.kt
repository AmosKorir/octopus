...
class FollowerRemoteMediator(
...
) : RxRemoteMediator<Int, FollowerDBModel>() {
    override fun loadSingle(
        loadType: LoadType,
        state: PagingState<Int, FollowerDBModel>
        ...
        return Single.just(loadType)
            ...
            .map {
                ...
                if (page == INVALID_PAGE) {
                    ...
                    userApi.getFollowers(username, page)
                        .subscribeOn(Schedulers.io())
    ...
    private fun insertToDb(followers: List<Follower>, loadType: LoadType, page: Int): List<Follower> {
        ...
        for (fw in followers){
          followerDao.insertFollower(FollowerDBModel(fw.id,fw.login,username,fw.avatar_url))
        }

        return followers
    }

    private fun getRemoteKeyForLastItem(state: PagingState<Int, FollowerDBModel>): RemoteKeyDbModel? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let { repo ->
           remoteKeysDao.getByIDRemoteKeys(repo.id).blockingGet()[0]
        }
    }

    private fun getRemoteKeyForFirstItem(state: PagingState<Int, FollowerDBModel>): RemoteKeyDbModel? {
    ...
    private fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, FollowerDBModel>): RemoteKeyDbModel? {
